...
train_generator = train_datagen.flow_from_directory(
   '../datasets/cats-dogs/train',  # this is the target directory
...
plt.savefig('results/100 Epochs/accuracy-binary-animal-classifier-v1-100-epochs.png')
...
plt.xlabel('epoch')
plt.legend(['train', 'validation'], loc = 'upper left')
plt.savefig('results/100 Epochs/loss-binary-animal-classifier-v1-100-epochs.png')

# Serialise the model to JSON
model_json = model.to_json()
with open("results/100 Epochs/animal-classifier-100-epochs.json", "w") as json_file:
   json_file.write(model_json)

# Serialise weights to HDF5
datestamp = datetime.datetime.now()
model.save_weights("results/100 Epochs" + str(
   datestamp) + ".h5")  # always save your weights after training or during training
